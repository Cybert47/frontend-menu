// - Gestor de pedidos para el panel de administración
...
class OrderManager {
  ...
  renderOrders() {
    ...
    if (this.currentFilter === "pending") {
      filteredOrders = filteredOrders.filter((order) => order.status === CONFIG.ORDER_STATES.PENDING)
    } else if (this.currentFilter === "completed") {
      filteredOrders = filteredOrders.filter((order) => order.status === CONFIG.ORDER_STATES.COMPLETED)
    }
    ...
    this.ordersContainer.innerHTML = filteredOrders
      .map((order) => {
        ...
        const isPending = order.status === CONFIG.ORDER_STATES.PENDING
  ...
  async completeOrder(orderId) {
    try {
      ...
      if (orderIndex !== -1) {
        this.orders[orderIndex].status = CONFIG.ORDER_STATES.COMPLETED
      }
  ...
  async showOrderDetails(orderId) {
    try {
      // Obtener el pedido de la lista local
      ...
      modalContent.innerHTML = `
        ...
        <div class="order-customer-info">
          ...
          <p><strong>Estado:</strong> ${order.status === CONFIG.ORDER_STATES.PENDING ? "Pendiente" : "Atendido"}</p>
        </div>
